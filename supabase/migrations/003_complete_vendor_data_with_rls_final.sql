...
-- Add reviews for featured vendors (using correct column names)
INSERT INTO vendor_reviews (vendor_id, reviewer_name, reviewer_email, rating, title, comment, is_verified, helpful_count)
SELECT
  v.id,
  r.reviewer_name,
  r.reviewer_email,
...
CROSS JOIN (
  VALUES
    ('John Smith', 'john@example.com', 5, 'Excellent Service!', 'Outstanding work and very professional. Highly recommend!', true, 12),
    ('Sarah Johnson', 'sarah@example.com', 4, 'Great experience', 'Very reliable and responsive. Will use again.', true, 8),
    ('Mike Davis', 'mike@example.com', 5, 'Top notch!', 'Best service provider we have worked with. Always on time and thorough.', true, 15)
) AS r(reviewer_name, reviewer_email, rating, title, comment, is_verified, helpful_count)
