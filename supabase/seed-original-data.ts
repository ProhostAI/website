...
async function seedOriginalData() {
  try {
    ...
    // Insert vendors with category assignments
    for (const vendor of originalVendors) {
      console.log(`Adding vendor: ${vendor.name}`);

      // Determine category based on services
      let categoryId = null;
      if (vendor.services.includes('Cleaning')) {
        categoryId = categoryMap['cleaning-services'];
      } else if (vendor.services.includes('Maintenance') || vendor.services.includes('Repairs')) {
        categoryId = categoryMap['maintenance-repairs'];
      } else if (vendor.services.includes('Pool Service')) {
        categoryId = categoryMap['pool-services'];
      } else if (vendor.services.includes('Landscaping')) {
        categoryId = categoryMap['landscaping'];
      } else if (vendor.services.includes('HVAC')) {
        categoryId = categoryMap['hvac-services'];
      } else if (vendor.services.includes('Photography')) {
      ...
      // Add some sample reviews for featured vendors
      if (vendor.is_featured && insertedVendor) {
        ...
        await supabase.from('vendor_reviews').insert(sampleReviews);
      }

      // Add gallery images for some vendors
      if (vendor.is_verified && insertedVendor) {
        ...
        await supabase.from('vendor_gallery').insert(galleryImages);
      }
      ...
      if (vendor.is_featured && insertedVendor) {
        ...
        await supabase.from('vendor_social_links').insert(socialLinks);
      }
    }
